...
type Agent struct {
	...
	// GeneratedConfigMapName is the name of the configmap generated by the Vault Agent
	// injector.
	...
	// InitConfig holds the generated Vault Agent init container configuration file
	// if applicable.
	InitConfig string
	...
	// SidecarConfig holds the generated Vault Agent sidecar container configuration file
	// if applicable.
...
func (a *Agent) Patch() ([]byte, error) {
	...
	if a.ConfigMapName != "" {
		a.Patches = append(a.Patches, addVolumes(
			a.Pod.Spec.Volumes,
			[]corev1.Volume{a.ContainerConfigMapVolume(a.ConfigMapName)},
			"/spec/volumes")...)
	} else {
		a.Patches = append(a.Patches, addVolumes(
			a.Pod.Spec.Volumes,
			[]corev1.Volume{a.ContainerConfigMapVolume(a.GeneratedConfigMapName)},
			"/spec/volumes")...)
	}
	...
	if a.PrePopulate {
		...
		if a.ConfigMapName == "" {
			config, err := a.newConfig(true)
			if err != nil {
				return patches, err
			}
			a.InitConfig = string(config)
		}
	...
	if !a.PrePopulateOnly {
		...
		if a.ConfigMapName == "" {
			config, err := a.newConfig(false)
			if err != nil {
				return patches, err
			}
			a.SidecarConfig = string(config)
		}
	...
	annotations[AnnotationAgentStatus] = "injected"

	if a.ConfigMapName == "" {
		annotations[AnnotationAgentGeneratedConfigMapName] = a.GeneratedConfigMapName
	}

	// Add annotations so that we know we're injected and optionally add generated
	// configmap name.
